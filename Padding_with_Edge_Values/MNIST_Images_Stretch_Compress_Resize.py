...
for iter in range(0, 60000, 1):
    ...
    height = np.random.poisson(28,1) * np.random.uniform(0.1,3)
...
# Resizing Training images back to 28x28

train_img22 = np.zeros((60000, 28, 28), dtype='uint8')

for it in range(0, 60000, 1):
    tmp    = train_img[it,:,:]
...
# Resizing Test images back to 28x28

test_img22 = np.zeros((10000, 28, 28), dtype='uint8')

for it in range(0, 10000, 1):
    tmp    = test_img[it,:,:]
    img    = imresize(tmp, (28, 28))
    test_img22[it,:,:] = img
...
np.save('train_img_2_2.npy', train_img22)
np.save('train_lab_2_2.npy', train_lab)

np.save('test_img_2_2.npy', test_img22)
np.save('test_lab_2_2.npy', test_lab)
